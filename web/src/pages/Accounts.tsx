...
export default function Accounts() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">AWS Accounts</h1>
        <p className="text-sm text-slate-500 mt-0.5">Connect your AWS account to start scanning</p>
      </div>

      {!acc && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 max-w-md">
          <h2 className="font-semibold text-slate-900 mb-1">Connect an AWS account</h2>
          <p className="text-sm text-slate-500 mb-4">Deploy a read-only IAM role and start scanning for IAM issues.</p>
          <button
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-2xl">
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {acc.account_id && <div className="text-xs text-slate-400 font-mono">{acc.account_id}</div>}
              </div>
            </div>
                    ...
                    content: (
                      <div className="space-y-2">
                        <div className="text-xs text-slate-400">
                          ExternalId: <code className="font-mono bg-slate-100 px-1.5 py-0.5 rounded">{acc.external_id}</code>
                        </div>
                        <div className="flex gap-2">
                          <input
                            className="flex-1 border border-slate-200 rounded-lg px-3 py-2 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
          ...
          {acc.status === "connected" && (
            <div className="px-6 py-5 space-y-3">
              <p className="text-sm text-slate-500">Account connected. Run a scan to detect IAM issues.</p>
              <div className="flex items-center gap-3">
                <button
                ...
                {scanQueued && (
                  <span className="text-sm text-slate-500">Scan queued — check Findings in ~30s</span>
                )}
              </div>
            </div>
          )}
