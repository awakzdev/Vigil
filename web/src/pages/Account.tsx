  ...
  return (
    <div className="max-w-lg space-y-8">
      <div>
        ...
        {me && <p className="text-sm text-slate-500 mt-1">{me.email}</p>}
      </div>
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        <p className="text-xs text-slate-500 mb-4">Connect GitHub to sign in without a password.</p>
        ...
        {me?.github_id ? (
          ...
          <a
            href={ghConnectUrl}
            className="inline-flex items-center gap-2 border border-slate-200 rounded-lg px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors"
          >
      ...
      {/* TOTP placeholder */}
      <section className="bg-white rounded-xl border border-slate-200 p-6 opacity-60">
        <h2 className="text-sm font-semibold text-slate-900 mb-1">Two-factor authentication</h2>
        <p className="text-xs text-slate-500">Authenticator app (TOTP) — coming soon.</p>
      </section>
    </div>
  );
}
