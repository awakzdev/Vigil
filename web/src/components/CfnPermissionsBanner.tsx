...
export default function CfnPermissionsBanner({ cfnLaunchUrl, className }: CfnPermissionsBannerProps) {
  return (
    <div
      ...
      role="status"
    >
      <p className="font-semibold">No data from newer AWS service collectors</p>
      <p className="mt-1 text-xs leading-relaxed text-amber-900/90">
        The latest scan returned zero Lambda, DynamoDB, ACM, ELB, SNS/SQS, Secrets Manager, SSM, snapshots, and AMIs.
        That usually means those resources are not in this account — not a broken connection. If you <em>do</em> use
        those services and still see this after a re-scan, update the read-only role stack and scan again.
      </p>
      {cfnLaunchUrl && (
        <a
          href={cfnLaunchUrl}
